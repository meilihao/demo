...
func sendHearbeet() {
	...
	for {
		...
		err := unix.Sendto(fd, buildMsg("hello meilihao!"), 0, &unix.SockaddrNetlink{
			Family: unix.AF_NETLINK,
			Pid:    NETLINK_PORT, // Pid不为0且使用新socket时不需要root权限
		})
		if err != nil {
			log.Fatal(err)
		}
...
// 使用原有socket发送msg
func sendHearbeet2(fd int) {
	for {
		log.Info("start send msg")

		err := unix.Sendto(fd, buildMsg("hello meilihao!"), 0, &unix.SockaddrNetlink{
			Family: unix.AF_NETLINK,
			Pid:    NETLINK_PORT, // Pid不为0且使用原有socket(即fd对应的socket)时需要root权限
		})
		if err != nil {
			log.Fatal(err)
		}
